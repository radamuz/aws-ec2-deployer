...
if [[ "$DOCKERFILE_PATH" == *postgres* || "$DOCKERFILE_PATH" == *mariadb* || "$DOCKERFILE_PATH" == *oracle* || "$USER_DATA_FILE" == *postgres* || "$USER_DATA_FILE" == *mariadb* || "$USER_DATA_FILE" == *oracle* ]]; then
  ...
  echo -e "${YELLOW}A database has been detected in the Dockerfile or user data file. Would you like to set up the infranettone database?${NC}"
  ...
  echo -e "${GREEN}End of block Would you like to set up the infranettone database?${NC}"
  ...
  echo -e "${GREEN}End of block If it is a PostgreSQL database.${NC}"
  # End of block If it is a PostgreSQL database.

fi
echo -e "${GREEN}End of block Detect whether a database has been deployed.${NC}"
